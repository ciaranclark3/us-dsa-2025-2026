...
imported from `lab05\base`, and it's defined for generic types `Key` and 
`Value`, where `Key` must implement the `Comparable<Key>` interface (see
`lab05\README.md` for a refresher on what this means).
...
the $\Theta(n \log n)$ algorithm we saw in Lecture 12 to build a binary search 
tree from an array of items.

Similarly to last week's `BinaryTree<Key, Value>` class, this class also
contains a nested class, `Node<Key, Value>`. Indeed, 
`BinarySearchTree<Key, Value>` features several methods, all of which are 
already implemented, and most of them call analogous methods defined in the
...
interface in the `BinarySearchTree<Key, Value>` outer class returns 
`this.findNode(key).item`, that is it calls the `findNode(key)` method just 
above it, which, in turn, calls `this.root.findNode(key)`. And *this* is the
method you need to implement, down in the `Node<Key, Value>` class. It is 
supposed to return an object of type `Node<Key, Value>`, namely the node in the
subtree of `this` node (the one calling this method we're implementing) whose
item's key is the input `key`. This method corresponds to `subtreeSearch(x,k)` 
that we have seen in Lecture 11, and it works essentially like binary search:
compare `key` with`this.item.key()` (using `key.compareTo(this.item.key())`): if
the result is positive and `this.right` is not `null`, then recursively return
...
worry if you don't know what exceptions are for now). If the result of the 
comparison is negative, do something analogous to the above; and just return 
...
The `insert(item)` method from the `OrderedMap<Key, Value>` interface in the
`BinarySearchTree<Key, Value>` outer class creates a new root node containing 
the input `item` if the current tree is empty, else it calls 
`this.root.insert(item)` from the `Node<Key, Value>` class.
...
The `remove(key)` method from the `OrderedMap<Key, Value>`interface in the
`BinarySearchTree<Key, Value>` outer class first calls `this.findNode(key)` to 
find the node whose item's key is `key` (if unsuccessful, this call will throw 
an exception), saves the result in a variable `node`, stores a copy of 
`node.item` in another variable `item`, calls `node.remove()` from the 
`Node<Key, Value>` class, and finally returns `item`.
...
method corresponds to the `removeNode(z)` procedure we saw in Lecture 12, where
...
`minNode()` and `maxNode()` of the `Node<Key, Value>` class, further down the 
file, so make sure to implement them first. (Again we discussed this in Lecture
12, where you can find the very simple pseudocode if you're struggling). Also,
remember to update `this.tree.size` by decreasing it before actually returning 
the removed node.
...
interface in the `BinarySearchTree<Key, Value>` outer class call 
`this.root.previous(key)` and `this.root.next(key)` respectively, from the
`Node<Key, Value>` class. In Lecture 12 we presented the pseudocode for 
`next(key)`, all you need to do here is to translate it into Java, again 
remembering that keys are comparable. Are you able to amend the code of 
`next(key)` to make it work correctly for `previous(key)` instead?
