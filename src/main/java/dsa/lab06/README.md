...
* A Java array `items` of objects of type `MapItem<Key,Value>`. This is our 
  backing array corresponding to $A$ in the lectures.
...
subclass of `BinaryTree<Item>` further down the file. Before analysing this 
subclass, however, have a look at the methods of `BinaryTree<Item>`: there are
quite a few, all implemented, and most of them (such as `height()`, `size()`,
`printPreOrder()`, `printInOrder()`, `printPostOrder()`) call methods from the
`Node<Item>` subclass on the node `root`.
...
Both of them are left for you to implement, and they should only require one 
line of code. `calculateSize()` requires you to calculate the number of nodes
stored in the subtree rooted at this node, assuming you already know the answer 
for this node's left and right children. At the moment it returns 0: you need to
change this returned value to the correct one. `calculateHeight()` has to be 
implemented similarly. These methods correspond to the `height(node)` and
`subtreeSize(node)` procedures we asked you to implement in Problem Sheet 5, so 
if you need a hint, go have a look at the solutions for that.

There is also a method `level()` which is for you to implement. Remember that 
the level of a node coincides with its depth, which is defined to be 0 for the
root, 1 for the root's children, 2 for the root's children's children, and so 
on. What you need to do is to figure out how to translate this intuitive idea in
actual code that calculates the level of this node correctly! You might need to 
use the already implemented `hasParent()` method...
...
type `Node<Item>`, you can only recursively call them on non-`null` children.
Therefore, make sure to use the already implemented `hasLeft()` and `hasRight()`
methods.
