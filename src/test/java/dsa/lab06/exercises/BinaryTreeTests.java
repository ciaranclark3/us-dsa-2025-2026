...
import dsa.lib.utils.StringUtils;
import dsa.lab06.data.BinaryTreeData;
...
public class BinaryTreeTests
{
  ...
  static Source<Arguments> arguments()
  {
    return BinaryTreeData.NON_EMPTY.replace((tree, index) ->
    {
      ...
      name.append("node ");
      name.append(ToStringUtils.toString(node.item()));
      name.append(" in ");
      name.append(tree.toCompactString());
      return argumentSet(name.toString(), tree, node);
    });
  }
  //</editor-fold>
  ...
  private static String captureOutput(Runnable runnable)
  {
    ...
    {
      lines[i] = StringUtils.toString(lines[i]);
    }
